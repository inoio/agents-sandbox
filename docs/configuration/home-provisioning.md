...
In addition to the agent config, the `home:` key provisions arbitrary files into the VM home directory (`/home/dev`). A
manifest is an optional YAML map from a **VM-home-relative target path** to a **host source string**, nested under the
`home:` key in the launcher config file:

| Config location                        | Purpose                          |
|----------------------------------------|----------------------------------|
| `~/.config/opencode-sandbox/config.yaml` (`home:`) | User-level home-file mappings    |
| `.opencode-sandbox/config.yaml` (`home:`)          | Project-level home-file mappings |

> **Note:** legacy `home.yaml` files are still honored but deprecated; migrate their entries into the `home:` key. The
> two are merged, with the `home:` key taking precedence over `home.yaml` for the same target.
...
Example project-level `home:` block in `.opencode-sandbox/config.yaml`:
