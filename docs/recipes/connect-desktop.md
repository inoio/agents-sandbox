...
> **Requires a daemon-based agent.** `--serve-only` starts the agent's server inside the VM, so it only works with
> daemon-based agents (opencode, opencode2); it is rejected for `pi` and `claude-code`. Opencode Desktop connects to an
> opencode server, so this recipe is opencode-specific.

agents-sandbox runs a headless `opencode serve` daemon inside the VM, and the
...
```console
agents-sandbox run --serve-only
```

Connect Opencode Desktop to the printed `http://127.0.0.1:<port>` URL — the
port is dynamically allocated, so always use the URL agents-sandbox prints.
...
project. So agents-sandbox always runs the server with the project mounted at
`/workspace`, and any client — TUI or Desktop — must address that same path.

## Connecting Opencode Desktop to the sandbox

1. Start the sandbox with `agents-sandbox run --serve-only`.
