...
Task 7: COMPLETE (commit da115d5 on opencode/sandbox-modularization-rework; base d5b97ce). Implementer DONE, reviewer Approved (spec ✅, no Critical/Important). VolumeAction typed enum (ActionKeep/Migrate/Reset/Quit + String + FromKey) with byte-identical prompt; session/reconfig.go "4"→volume.ActionQuit; ArtifactInfo moved naming→artifact; FindHashSuffix→findHashSuffix (+ prune_test external dup removed, coverage kept in-package); IsStoppedStatus=!IsSandboxActive (unknown-status flip is plan-mandated, documented); uint32 default retypes; stale comments fixed; implementer correctly KEPT newDefaultErrorDockerClient (not a dead branch). Minor (deferred): (1) TestVolumeActionString misses default/"unknown" branch; (2) FromKey error discard in ResolveHomeAction (safe, zero-value default) — comment would help; (3) possible unnecessary //nolint:revive on VolumeAction (verify lint fires; else remove). Parked for final review triage.

Task 8: COMPLETE (commit 022105b on opencode/sandbox-modularization-rework; base d3385d3). First implementer interrupted mid-task, leaving a DOUBLE-CLIENT-LEASE bug in the new runAttach (Run non-serve held a lease at top-of-Run AND inside runAttach, so reap ran with the client's lease still held). Fresh implementer fixed: lease moved into the ServeOnly-only branch; non-serve + Shell each hold exactly ONE lease inside runAttach, explicitly released BEFORE reap with nil-gated deferred safety net. Reviewer Approved after fix (spec ✅, lease ordering verified against CountActiveClients/reapOnLastClient; no Critical/Important). Steps: referencesImage(tag) dedup; runAttach extraction; NewHomeState (3 call sites); EnvState/SecretState = FingerprintState aliases (on-disk YAML identical). Minor (deferred): //nolint:nestif on ServeOnly branch in Run — extracting runServeOnlyClient would remove nesting (optional). Parked for final review triage.

Task 9: COMPLETE (commit 0d37642 on opencode/sandbox-modularization-rework; base 71f1a39). Implementer DONE, reviewer Approved (spec ✅, no Critical/Important). All `var Fn = sub.Fn` mutable re-exports converted to func wrappers with EXACT submodule signatures (Prune 4-param, AutoPrune, EnsureImage, GetConfigPaths, ListVolumes, CmdMigrate/Reset/Edit); reexport_query.go merged into reexport_session.go (Info/ImageInfo/ListImages); msb_client.go trimmed to remove EXACTLY the 4 zero-reference aliases (TestFS, TestResult, SandboxFS, MockRemoveImageCall) — cmd test mocks (MockMsbClient, WithMsbMock, NewTestResult, MockSandbox*, etc.) retained; reexport doc comments standardized; no cmd/ files touched, no import cycles. No deferred minors. CLI surface fully preserved.
