...
def test_branch_slug_replaces_slashes():
    assert worktree.branch_slug("feature/login") == "feature-login"
    assert worktree.branch_slug("feature/sub/deep") == "feature-sub-deep"
    assert worktree.branch_slug("main") == "main"
...
def test_project_slug_from_git_common_dir(tmp_path):
    ...
    with patch("pathlib.Path.cwd", return_value=tmp_path):
        with patch("subprocess.check_output", return_value=str(common_dir).encode() + b"\n"):
...
def test_project_slug_fallback(tmp_path):
    ...
    with patch("pathlib.Path.cwd", return_value=non_git):
        with patch("inoio_sandbox.log.warn") as mock_warn:
            slug = worktree.project_slug()
            assert slug.startswith("p-")
            assert len(slug) == 10  # p- + 8 hex chars
...
def test_current_worktree_path(tmp_path):
    ...
    top_level.mkdir()
    with patch("subprocess.check_output", return_value=str(top_level).encode() + b"\n"):
...
def test_ensure_worktree_idempotent(tmp_path):
    ...
    with (
        ...
        patch("inoio_sandbox.worktree._is_git_worktree", return_value=True),
    ):
        result = worktree.ensure_worktree(repo_root, state_dir, "p-deadbeef", "feature-x")
        assert result == target
...
def test_ensure_worktree_recreates_stale_dir(tmp_path):
    state_dir = tmp_path / "state"
    repo_root = tmp_path / "repo"
    repo_root.mkdir()
    target = worktree.worktree_path(state_dir, "p-deadbeef", "feature-x")
    ...
    with (
        patch("subprocess.run") as mock_run,
        patch("inoio_sandbox.worktree._is_git_worktree", return_value=False),
