...
func TestExecuteTree(t *testing.T) {
	...
	if ui.InfoCalls[0] != "opencode-sandbox" {
...
func TestMainProcess(t *testing.T) {
	t.Run("version exits zero with output", func(t *testing.T) {
		...
		if !strings.Contains(out, "opencode-sandbox") {
...
func TestMainHelperProcess(t *testing.T) {
	...
	configpaths.WithMockConfigPaths(t)
	os.Args = append([]string{"opencode-sandbox"}, args...)
	main()
	os.Exit(0)
}
...
func TestMainHelperProcessExitError(t *testing.T) {
	...
	os.Args = []string{"opencode-sandbox", "run"}
	main()
	os.Exit(0)
}
