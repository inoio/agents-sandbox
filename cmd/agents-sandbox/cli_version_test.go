...
func TestVersion(t *testing.T) {
	...
	t.Run("default version is dev", func(t *testing.T) {
		...
		if !strings.Contains(testUI.OutCalls[0], "agents-sandbox dev") {
			t.Errorf("expected version output to contain %q, got %q", "agents-sandbox dev", testUI.OutCalls[0])
		}
	})
	...
	t.Run("custom version is displayed correctly", func(t *testing.T) {
		...
		if !strings.Contains(testUI.OutCalls[0], "agents-sandbox 1.2.3") {
			t.Errorf("expected version output to contain %q, got %q", "agents-sandbox 1.2.3", testUI.OutCalls[0])
		}
	})
}
